  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 bg-gray-900 min-h-screen text-white">
      ...
      <div>
        <button 
          onClick={() => navigate(-1)}
          className="flex items-center text-yellow-400 mb-6 hover:text-yellow-300"
        >
      ...
      <div className="mb-8">
        ...
        >
          {[1, 2, 3, 4, 5, 6, 7, 8].map(num => (
            <option key={num} value={num} className="text-white bg-gray-800">{num}</option>
          ))}
        </select>
      </div>
      ...
      <div className="grid grid-cols-10 gap-2 mb-8">
        {seats.map(seat => (
          <button
            ...
            `}
          >
            <FaChair className={!seat.available ? 'text-gray-500' : ''} />
